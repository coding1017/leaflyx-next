...
const hasProfanity = (s: string) => BAD_WORDS.some((w) => s?.toLowerCase().includes(w));
...
export async function GET(req: Request) {
  try {
    ...
    const [total, items, grouped, avg] = await withRetry(() =>
      Promise.all([
    ...
    // If DB blips even after retries, return a safe payload so UI doesn't implode
