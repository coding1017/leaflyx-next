// app/admin/products/quick-create/page.tsx
"use client";
...
function normalizeCategoryToSlug(c: string) {
  return String(c || "").trim().toLowerCase();
}

const TOKEN_KEY = "leaflyx_admin_token";

export default function AdminProductsQuickCreatePage() {
  ...
  async function submit() {
    ...
    try {
      ...
      fd.set("category", normalizeCategoryToSlug(category));
  ...
  function addVariant() {
    setVariants((prev) => [...prev, { id: "", label: "", grams: undefined, price: undefined, isPopular: false, initialQty: 0 }]);
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10 text-white">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold">Admin Products — Quick Create</h1>
          <p className="text-sm opacity-80 mt-1">
            Legacy builder. Shopify-style page:{" "}
            <Link href="/admin/products/new" className="text-[var(--brand-gold)] hover:underline">
              /admin/products/new
            </Link>
          </p>
        </div>

        <Link href="/admin/products" className="rounded-xl border border-white/15 bg-black/30 px-4 py-2 text-sm text-white/80 hover:text-white">
          Back to list
        </Link>
      </div>

      <div className="mt-6 rounded-2xl border border-[var(--brand-gold)] bg-[#0f2a1f] p-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-end">
          ...
          <button onClick={saveToken} className="rounded-xl bg-white/10 px-4 py-2 text-sm hover:bg-white/15">
            Save token
          </button>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-white/10 bg-black/30 p-4 space-y-3">
          ...
          <Field label="ID (optional)" value={id} onChange={setId} placeholder="(leave blank to auto-generate)" />
        ...
        <div className="rounded-2xl border border-white/10 bg-black/30 p-4 space-y-3">
          ...
          <label className="block">
            ...
            <div className="text-xs opacity-60 mt-1">Uploads to Blob and stores URL in DB.</div>
          </label>
              ...
              <div key={i} className="rounded-xl border border-white/10 bg-black/20 p-3">
                <div className="grid gap-2 md:grid-cols-2">
                  ...
                  <label className="flex items-center gap-2 text-sm opacity-80 mt-6">
                    <input type="checkbox" checked={!!v.isPopular} onChange={(e) => updateVariant(i, { isPopular: e.target.checked })} />
                    Popular
                  </label>
                </div>

                <button onClick={() => removeVariant(i)} className="mt-3 rounded-lg bg-red-500/15 px-3 py-2 text-xs text-red-200 hover:bg-red-500/25">
                  Remove variant
                </button>
              </div>
            ))}
          ...
          <button onClick={addVariant} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs hover:bg-white/10">
            Add variant row
          </button>

          <div className="mt-3">
            <div className="text-xs opacity-70 mb-1">variantsJson (what will be sent)</div>
            <pre className="rounded-xl border border-white/10 bg-black/30 p-3 text-xs overflow-x-auto">{variantsJson}</pre>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center gap-3">
        <button
          disabled={status === "saving" || !token || !slug || !name || !category}
          onClick={submit}
          className="
